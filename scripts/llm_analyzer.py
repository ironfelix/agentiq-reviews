...
СИСТЕМНЫЕ ПРОБЛЕМЫ (важно замечать паттерны):
Если видишь НЕСКОЛЬКО отзывов про одну и ту же проблему — это СИСТЕМНАЯ проблема, НЕ единичный случай.
Продавец ОБЯЗАН признать это и обещать улучшить описание товара.

Универсальные системные проблемы (для всех категорий):
• Упаковка: "помято", "рванный пакет", "грязное", "заклеен скотчем"
  → "Нам очень жаль за состояние упаковки — это недопустимо. Мы передали информацию в логистику для проверки."

• Дефект/брак: "сломано", "не работает", "брак", "царапины"
  → "Нам очень жаль за эту ситуацию! Такое недопустимо — мы передали информацию в отдел качества для проверки партии. Оформите, пожалуйста, возврат через ЛК WB как 'не соответствует описанию' — это бесплатно."

• Несоответствие описанию: "не то что на фото", "другой цвет", "не те характеристики"
  → "Нам жаль за это несоответствие. Мы проверим фото и описание товара — ваш отзыв поможет улучшить карточку."

Категория-специфичные проблемы (если применимо):
• Fashion (одежда/обувь): "маломерит", "большемерит", "тонкая ткань", "просвечивает", "цвет не тот"
  → Признай + "учтём для размерной таблицы/описания" + совет будущим покупателям

• Электроника: "быстро разряжается", "перегревается", "не держит заряд"
  → Объясни факторы (яркость, температура, режим) + дай совет по использованию. Если дефект — возврат.

• Продукты/корма: "срок годности", "вкус не тот", "состав отличается"
  → Признай + "проверим партию" + возврат если дефект

• Детские товары: "маленький размер", "неудобно", "безопасность"
  → Особая эмпатия + признание + возврат если проблема безопасности

• Косметика: "аллергия", "запах не тот", "консистенция"
  → Если аллергия — ЭСКАЛАЦИЯ К МЕНЕДЖЕРУ (медицинский риск), не отвечай автоматически

ВАЖНО: Если 2+ отзыва про одно и то же — ОБЯЗАТЕЛЬНО упомяни системность в verdict и рекомендациях.
Шаблонный ответ "надеемся вы найдёте другую модель" на системную проблему = harmful (ignore)."""
...
ФОРМУЛА РАСЧЁТА quality_score (1-10) — ПРОЦЕНТНАЯ:
1. Рассчитай процент каждой категории от total_analyzed
2. harmful_pct = (harmful / total) × 100
3. risky_pct = (risky / total) × 100
4. good_pct = (good / total) × 100
5. quality_score = 10 - (harmful_pct × 0.1) - (risky_pct × 0.05) + (good_pct × 0.02)
6. Минимум = 1, максимум = 10

ПРИМЕРЫ РАСЧЁТА:
• 100 ответов: 70 acceptable, 20 good, 10 risky → harmful=0%, risky=10%, good=20% → 10 - (0×0.1) - (10×0.05) + (20×0.02) = 10 - 0 - 0.5 + 0.4 = 9.9 → ★★★★★★★★★☆ (10/10)
• 50 ответов: 40 acceptable, 5 harmful, 5 risky → harmful=10%, risky=10%, good=0% → 10 - (10×0.1) - (10×0.05) + 0 = 10 - 1 - 0.5 = 8.5 → ★★★★★★★★☆ (9/10)
• 200 ответов: 150 acceptable, 30 risky, 15 harmful, 5 good → harmful=7.5%, risky=15%, good=2.5% → 10 - (7.5×0.1) - (15×0.05) + (2.5×0.02) = 10 - 0.75 - 0.75 + 0.05 = 8.55 → ★★★★★★★★★☆ (9/10)
• 10 ответов: 7 acceptable, 2 harmful, 1 good → harmful=20%, risky=0%, good=10% → 10 - (20×0.1) - 0 + (10×0.02) = 10 - 2 + 0.2 = 8.2 → ★★★★★★★★☆ (8/10)
• 202 ответов: 179 acceptable, 10 good, 5 harmful, 8 risky → harmful=2.5%, risky=4%, good=5% → 10 - (2.5×0.1) - (4×0.05) + (5×0.02) = 10 - 0.25 - 0.2 + 0.1 = 9.65 → ★★★★★★★★★★ (10/10)

ВАЖНО: Оцени по ПРОЦЕНТУ вредных, а не по абсолютному числу. 5 harmful из 200 (2.5%) = отлично. 5 harmful из 10 (50%) = катастрофа.
Балл 1-3 = катастрофа (>70% harmful). Балл 4-5 = плохо (40-70% harmful). Балл 6-7 = средне (10-40% harmful). Балл 8-10 = хорошо (<10% harmful).

Верни JSON:
{{
  "quality_score": число 1-10 (рассчитай строго по формуле выше),
  ...
  "distribution": {{
    "harmful": число (ответы, которые вредят продажам),
    "risky": число (ответы с проблемами),
    "acceptable": число (нормальные, не вредят),
    "good": число (хорошие ответы)
  }},
  ВАЖНО: distribution ДОЛЖНА покрывать ВСЕ проанализированные отзывы с ответами.
  harmful + risky + acceptable + good = total_analyzed.
  Классифицируй ВСЕ ответы, включая ответы на позитивные отзывы (4-5★).
  Шаблонный ответ на позитив = acceptable (не вредит). Шаблон на негатив = harmful.
  "error_types": [
    {{
  ...
  "worst_responses": [
    {{
      "review_rating": число,
      "date": "YYYY-MM-DD из входных данных",
      "color": "цвет/вариант из входных данных или null",
      "review_text": "цитата отзыва (до 100 символов, обрежь по границе предложения)",
      "response_text": "цитата ответа продавца (до 150 символов, обрежь по границе предложения, добавь «…» если обрезано)",
      ...
      "recommendation": "ГОТОВЫЙ ТЕКСТ ответа от лица продавца (2-3 предл.). Пиши именно текст, который продавец должен был написать покупателю. НЕ инструкция, НЕ описание — а сам ответ. Пример: «Нам очень жаль! Оформите возврат через личный кабинет WB, мы учтём ваш отзыв.»",
      "recommendation_reason": "почему именно такой ответ лучше (1 предл.)"
    }}
  ],
  "hidden_risks": [
    {{
      ...
      "review_text": "цитата жалобы из позитивного отзыва (до 100 символов, обрежь по границе предложения)",
      "response_text": "цитата ответа продавца (до 150 символов, обрежь по границе предложения, добавь «…» если обрезано)",
      "issue": "что именно проигнорировано"
    }}
  ],
  ВАЖНО для hidden_risks: включай ТОЛЬКО случаи, когда продавец ПОЛНОСТЬЮ проигнорировал жалобу (отвечает про другое или шаблон «спасибо»). Если продавец АДРЕСОВАЛ проблему (объяснил, извинился, предложил помощь) — это НЕ hidden_risk, даже если решение неконкретное.
  "buyer_perception": [
...
8. recommendation — это ГОТОВЫЙ ТЕКСТ ответа от лица продавца, а НЕ инструкция/совет. Неправильно: «Следовало извиниться и предложить помощь». Правильно: «Нам очень жаль! Напишите нам, мы поможем разобраться.»
9. При классификации учитывай КОНТЕКСТ: шаблонный ответ на 5★ без жалоб = acceptable (не вредит). Тот же шаблон на 1★ с жалобой = harmful (игнорирует проблему). Ответ «Спасибо!» на позитив = acceptable, на негатив = ignore (harmful).
10. Адресование vs игнор жалобы:
   • АДРЕСОВАЛ (acceptable/good): «делаем все возможное», «учтём замечание», «свяжитесь с нами», «это зависит от WB логистики»
   • ИГНОР (harmful): «Спасибо за отзыв!» без упоминания проблемы, «рады что понравилось» когда жаловались
   Если продавец хотя бы упомянул проблему — это НЕ игнор."""
...
def llm_analyze_communication(
    ...
    for i, fb in enumerate(feedbacks):
        ...
        fb_date = (fb.get("fb_created_at") or "")[:10]  # YYYY-MM-DD
        ...
        meta_parts = [f"★{rating}"]
        if fb_date:
            meta_parts.append(fb_date)
        if color:
            meta_parts.append(color)

        lines.append(
            f"[{i+1}] {' | '.join(meta_parts)} | Отзыв: {review_text}\n"
            f"    Ответ: {answer_text}"
        )
    ...
    # Use larger max_tokens for comprehensive analysis (3072 to prevent truncation)
    raw = _call_llm(COMM_SYSTEM, user, max_tokens=3072)

    # DEBUG: Save raw response to file
    if raw:
        try:
            with open("/tmp/comm_llm_raw.txt", "w", encoding="utf-8") as f:
                f.write(raw)
            print(f"[Communication] Raw LLM response saved to /tmp/comm_llm_raw.txt ({len(raw)} chars)")
        except Exception as e:
            print(f"[Communication] Failed to save raw response: {e}")

    parsed = _parse_json_response(raw)

    if parsed is None:
        print(f"[Communication] JSON parsing failed. Raw response length: {len(raw) if raw else 0}")
        return None
...
def _apply_communication_guardrails(result: dict) -> dict:
    ...
    for resp in result.get("worst_responses", []):
        ...
        if isinstance(rec, str):
            ...
            # Check if recommendation is an instruction instead of a ready-made response
            instruction_markers = [
                "следовало ", "нужно было ", "стоило ", "необходимо было ",
                "продавцу следует ", "рекомендуется ", "важно было ",
                "следует выразить", "нужно выразить", "стоит выразить",
            ]
            rec_lower_check = rec.lower().strip()
            if any(rec_lower_check.startswith(m) for m in instruction_markers):
                print(f"[COMM GUARDRAIL] Recommendation is instruction, not seller text: '{rec[:80]}'")
    ...
    # --- 7. Validate distribution sums to total_analyzed ---
    dist = result.get("distribution", {})
    total_analyzed = result.get("total_analyzed", 0)
    dist_sum = sum(dist.get(k, 0) for k in ("harmful", "risky", "acceptable", "good"))
    if dist_sum < total_analyzed and total_analyzed > 0:
        # LLM didn't classify all reviews — put remainder into acceptable
        gap = total_analyzed - dist_sum
        dist["acceptable"] = dist.get("acceptable", 0) + gap
        result["distribution"] = dist
        print(f"[COMM GUARDRAIL] Distribution gap: {dist_sum}/{total_analyzed}, added {gap} to acceptable")

...
46. **BL-POST-013: Pessimistic Lock для Auto-Response (race condition)**
    - Goal: Исключить дублирование ответов если продавец отвечает одновременно с авто-ответчиком.
    - Problem: `process_auto_responses` и ручная отправка из чат-центра работают в разных процессах (Celery worker vs API). Теоретически оба могут отправить ответ на один и тот же отзыв/вопрос.
    - Tasks:
      - `SELECT ... FOR UPDATE SKIP LOCKED` при выборке interactions в `process_auto_responses`
      - Либо advisory lock по interaction_id перед отправкой
      - Тест: параллельный запуск auto + manual на одну interaction → только один ответ уходит
    - Risk: Низкий на текущих объёмах (max 10 авто-ответов / 3 мин), но нужно до масштабирования.
    - Estimate: 0.5 дня.

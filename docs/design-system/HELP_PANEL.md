...
Единый стандарт для продукта: панель всегда открывается **справа**.
Используется в `Промокоды`, `Главная (KPI контекст)`, и других разделах с inline-объяснениями.
...
### Dashboard KPI (right, same pattern)
```html
...
<div class="promo-help-panel" id="dashboardContextPanel">...</div>
```
...
4. Проверить mobile: `promo-help-panel` занимает ширину экрана.

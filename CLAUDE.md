...
## Cross-model Sync (ОБЯЗАТЕЛЬНО)
- Перед началом любой задачи прочитай **`CODEX.md`**.
- При обновлении ключевых правил синхронно обновляй **`CLAUDE.md`** и **`CODEX.md`**.

## Documentation Policy (ОБЯЗАТЕЛЬНО)
Примечание по путям: в этом репозитории каноническая документация лежит в `agentiq/docs/`. Во многих местах используются короткие пути вида `docs/...` в предположении, что рабочая директория это `agentiq/`.

1. Документация проекта хранится в `docs/` (внутри `agentiq/`) и сводится в:
- `docs/docs-home.html` (человеческий портал: навигация + статусы + карта модулей)
- `docs/INDEX.md` (Markdown-индекс)
2. Что дальше (как поддерживать актуальность). Чтобы документация не “протухала”, важна дисциплина:
- В каждом крупном документе в начале: `Last updated:` и `Status:`.
- Если это гипотеза: явно писать “это план, не реализовано”.
- Если это реализовано: давать ссылки на “истинные” файлы/эндпоинты/таблицы и текущие ограничения.
3. Execution log по Unified Communications держать в:
- `docs/product/UNIFIED_COMM_PLAN_V3_WB_FIRST.md`

## 1. ПЕРЕД ЛЮБОЙ РАБОТОЙ — прочитай контекст

### Общий контекст проекта
- **`CODEX.md`** — инструкции для Codex и правила межмодельной синхронизации
- **`docs/reviews/PROJECT_SUMMARY.md`** — архитектура, файлы, quickstart, API

### При работе с ответами на отзывы / communication analysis
- **`docs/reviews/RESPONSE_GUARDRAILS.md`** — banned phrases, формат ответов
- **`scripts/llm_analyzer.py:608-700`** — COMM_SYSTEM промпт + JSON формат
- **`scripts/llm_analyzer.py:478-519`** — GUARDRAILS конфиг (banned phrases, лимиты)
- **`docs/reviews/QUALITY_SCORE_FORMULA.md`** — формула quality_score (1-10)

### При работе с классификацией отзывов
- **`docs/reviews/reasoning-rules.md`** — классификация, пороги, примеры
- **`docs/reviews/CARD_FORMAT.md`** — формат карточек отчёта

### При работе с Chat Center
- **`docs/chat-center/chat-center-real-data.html`** — ЭТАЛОН дизайна (HTML + 1110 строк CSS)
- **`docs/chat-center/WB_CHAT_API_RESEARCH.md`** — исследование WB Chat API
- Прототипы: `docs/prototypes/chat-center-final.html`
- **`docs/prototypes/app-screens-v3-ru.html`** — основной source of truth для текущего app-flow (обязательно)

### Frontend Design Contract (ОБЯЗАТЕЛЬНО)
- Для реализации интерфейса приложения использовать строго `docs/prototypes/app-screens-v3-ru.html`.
- При оживлении сервиса (API, sync, состояние) не менять утвержденный дизайн, композицию и UX-паттерны.
- Допустимы только привязка данных и обработка состояний (loading/error/empty) без редизайна экранов.

### При работе с лендингом
- **`docs/prototypes/landing.html`** — основной source файл (~2050 строк)
- GitHub Pages: `docs/prototypes/landing.html` (путь зависит от репозитория)
...
3. **«Все сообщения»** — `chat_status === 'responded'` ИЛИ `chat_status === 'auto-response'` ИЛИ `chat_status === 'closed'`
...
### Chat Center — Жизненный цикл чата (chat_status)
```
waiting → responded → client-replied → closed
                   ↑__________________|
```
- **waiting** — новый чат или клиент написал, ждёт ответа продавца
- **responded** — продавец ответил, ждём реакции клиента
- **client-replied** — клиент ответил после ответа продавца
- **closed** — чат закрыт (вручную или auto-close через 10 дней неактивности)

**Автоматическое обновление:** `_recalculate_chat_status()` в `sync.py` при каждом новом сообщении.

### Chat Center — Интенты и SLA приоритеты (ai_analyzer.py)

**Post-purchase интенты:**
| Интент | Приоритет | SLA |
|--------|-----------|-----|
| `defect_not_working` | **urgent** | <30 мин |
| `wrong_item` | **urgent** | <30 мин |
| `delivery_problem` | high | <1 час |
| `return_refund` | high | <1 час |
| `missing_parts` | high | <1 час |
| `quality_complaint` | normal | <4 часа |
| `usage_question` | normal | <4 часа |
| `positive_feedback` | low | <24 часа |

**Pre-purchase интенты (HIGH priority — покупатель СЕЙЧАС на карточке!):**
| Интент | Приоритет | SLA (чат) | SLA (вопрос) |
|--------|-----------|-----------|--------------|
| `pre_purchase` | **high** | **<2 мин** (AI auto) | <5 мин |
| `sizing_fit` | **high** | **<2 мин** (AI auto) | <5 мин |
| `availability` | **high** | **<2 мин** (AI auto) | <5 мин |
| `compatibility` | **high** | **<2 мин** (AI auto) | <5 мин |

> Подробнее: [`docs/SLA_RULES.md`](docs/SLA_RULES.md) — окно решения покупателя 10-20 сек, 1 час = устаревший human benchmark

**Ключевые слова pre-purchase:**
- `sizing_fit`: размер, размерная сетка, какой размер, подойдёт ли
- `availability`: в наличии, есть ли, когда будет, поступление
- `compatibility`: совместим, подойдёт к, работает с, подходит для

### Chat Center — Filter Pills (левая панель)
Горизонтальные табы-фильтры поверх queue sections:

| Pill | Логика фильтрации | Примечание |
|------|-------------------|------------|
| **Все** | Без фильтра | Показывает все чаты |
| **Срочно** | `sla_priority === 'urgent' \|\| 'high'` | Client-side only |
| **Без ответа** | `unread_count > 0` | Server-side (`has_unread=true`) |
| **Обработаны** | `chat_status in ('responded', 'auto-response', 'closed')` | Client-side only |

**Empty state при фильтре:** если `chats.length > 0`, но `filteredChats.length === 0` — показывать "Нет чатов по фильтру" + кнопка "Показать все". НЕ показывать onboarding empty state (подключение API).

### Chat Center — Mobile (КРИТИЧНО)
- **Breakpoints:** `768px` (mobile), `769-1024px` (tablet)
- **Переключение панелей:** `data-mobile-view` атрибут на `.chat-center` — `list` / `chat` / `context`
- На mobile показывается ОДНА панель fullscreen, переключение через state в `App.tsx`
- Кнопка "Назад" (`.chat-header-back`) видна только на mobile (`display: none !important` на `>=769px`)
- **Product context** на tablet: скрыт по умолчанию, показывается по кнопке как floating overlay
- **Input font-size:** `>=16px` на mobile чтобы iOS не зумил
- **Touch targets:** минимум `44x44px` для кнопок и интерактивных элементов
- **Filter pills:** горизонтальный скролл через `.filters-scroll` с `overflow-x: auto`
- **Advanced filters:** `position: fixed; bottom: 0` как bottom sheet на mobile
- **CSS файл:** `frontend/src/index.css` строки 1091-1199 (mobile), 1201-1220 (tablet)
...
### Help Panel (UI паттерн, ОБЯЗАТЕЛЬНО)
- Используй единый компонент `promo-help-panel` для всех контекстных панелей (`Промокоды`, `Главная`, `Аналитика`, `Настройки`).
- Панель открывается **справа** (`right slide-out`), без альтернативного left-варианта.
- Триггер открытия ставь в верхнюю строку соответствующего блока (по паттерну `Как это работает?` в `SCREEN 7: PROMO CODES`).
- Внутренние классы панели: `promo-help-close`, `promo-help-title`, `promo-help-section`, `promo-help-label`, `promo-help-text`.
- На mobile панель должна занимать полную ширину (`width: 100%`).
- Подробности и примеры: `docs/design-system/HELP_PANEL.md`.
...
- **Landing sync:** после каждого изменения `docs/prototypes/landing.html` → синхронизировать в `apps/chat-center/frontend/landing.html`
...
- **CLAUDE.md НИКОГДА не коммитить** — содержит IP, SSH-ключи, JWT-токены, внутреннюю архитектуру. Всегда исключать из `git add`
- **Экономия токенов** — перед началом задачи предложи оптимальную модель (см. секцию ниже)

### Выбор модели — экономия бюджета (ОБЯЗАТЕЛЬНО)

Перед началом каждой задачи **предложи пользователю** какую модель лучше использовать. Формат: одна строка вида `💡 Рекомендация: /model sonnet — задача простая, экономим ~5x`.

| Задача | Модель | Почему |
|--------|--------|--------|
| Архитектура, сложный рефакторинг, продуктовый анализ | **Opus** (текущий) | Нужна глубина мышления |
| Написание нового кода, фичи, баг-фиксы | **Sonnet** (`/model sonnet`) | 5x дешевле, качество достаточное |
| Простые правки, CSS, copy-paste, переименования | **Sonnet** (`/model sonnet`) | Точно хватит |
| Поиск по кодовой базе, чтение файлов | **Haiku** subagents | 20x дешевле, идеально для grep/read |
| Ресерч, документация, структурирование | **Sonnet** | Хватает, не нужен Opus |
| Деплой, git операции, shell команды | **Sonnet** | Команды простые |

**Правила:**
- Всегда начинай сессию с Sonnet, переключай на Opus только если задача реально сложная
- Subagents для поиска — всегда `model: "haiku"`
- Новый чат при смене темы — не раздувай контекст (input tokens = основной расход)
- Если пользователь уже на Opus и задача простая — напомни про `/model sonnet`
...
- [x] **Celery Beat:** настроен на проде (systemd сервисы agentiq-celery, agentiq-celery-beat)
  ...
  - Auto-close неактивных чатов каждые 24 часа (10 дней без сообщений)

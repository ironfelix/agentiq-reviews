...
|-----------|-------------|---------|---------|
| **P0** | < 1 часа | `defect_not_working`, `wrong_item` | Клиент получил не то / брак |
| **P1** | < 1 часа | `delivery_delay`, `cancel_request` | Клиент раздражён, повторные сообщения |
| **P2** | < 4 часов | `refund_exchange`, `delivery_status` | Стандартный запрос |
| **P3** | < 24 часов | `usage_howto`, `product_spec` | Информационный вопрос |
